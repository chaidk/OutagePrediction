...
def save(data):#保存 打开文件
    data.to_excel('故障信息\data.xls', 'sheet1')
    os.system('故障信息\data.xls')
def user_gds(roudName):#供电所线路用户表
    df = pd.read_excel('故障信息\用户信息统计表.xls', '供电所线路用户', header=2)
    df = df.reset_index()
...
def own(roudName):#局属小区
    df = pd.read_excel('故障信息\用户信息统计表.xls', '局属小区', header=1, index_col=0)
...
def userGDS(roudName):#供电所线路用户表
    df = pd.read_excel('故障信息\用户信息统计表.xls', '供电所线路用户', header=2, index_col=0)
    df = df.reset_index(drop=True)
    df.fillna('-', inplace = True)
    rst = pd.DataFrame(columns=df.columns, index=None)
    for index in df.index:
